        ...
        input: [
          {
            role: "system",
            content:
              "너는 한국어 연애 커뮤니케이션 테스트 서비스 '연못'의 결과 분석가이자 카피라이터다. 사용자의 카톡 답장을 분석하되 모욕하지 말고, 직설적이고 밈처럼 재밌지만 실용적으로 말한다. 공감은 오구오구 달래기나 무조건 사과가 아니라, 상대가 왜 저 말을 했는지 읽고 내 입장과 경계를 관계가 깨지지 않게 표현하는 능력이다. 무조건 상대 감정을 받아주라는 식의 을의 연애 조언은 피하고, 호감 표현·자기 입장·경계 설정·대화 지속력의 균형을 본다. 진단은 의학/심리 치료처럼 단정하지 않는다. JSON만 출력한다."
          },
          {
            role: "user",
            content: JSON.stringify({
              instruction:
                "아래 8문항 응답들과 규칙 기반 결과를 바탕으로 최종 결과 JSON을 작성해줘. localResult는 기준선이므로 명백한 근거 없이 점수를 크게 벗어나게 주지 마라. 타입별 의미를 반드시 구분해라. free는 text가 사용자가 실제 보낸 답장이다. choice_reason은 selectedOption이 사용자의 선택이고 text/reasonText는 선택 이유이므로, 이유 문장을 '가장 위험했던 답장'으로 인용하지 말고 필요하면 selectedOption만 평가해라. rewrite는 text/rewrittenText가 사용자가 고친 문장이고 originalText는 원래 나쁜 예시이므로 사용자 답장처럼 비난하지 마라. empathy 점수는 상대 말의 의도 파악, 상황 인식, 적절한 확인 질문, 내 입장과 경계의 균형을 본다. 사과나 감정 받아주기만 많다고 높게 주지 말고, 반대로 자기 입장을 말한다고 낮게 주지 마라. '대화 맥락 이탈', '딴소리', '무시' 평가는 사용자가 명백히 연애 대화와 무관한 주제로 빠진 경우에만 써라. 짧거나 건조하거나 이유를 적은 문장을 맥락 이탈로 몰지 마라. 좋은 답변이 많으면 억지로 비난하지 말고 칭찬과 개선점을 섞어라. 점수는 공감/명확함/온도/여유를 0~100 정수로 주고, overall도 0~100 정수로 줘라.",
              answers,
              localResult
            })
          }
